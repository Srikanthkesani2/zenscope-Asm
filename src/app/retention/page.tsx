...
export default async function RetentionPage({ searchParams }: { searchParams: { from?: string; to?: string; source?: string } }) {
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="h-16 flex items-center justify-between gap-4">
            ...
            <nav className="flex gap-4 sm:gap-6 text-sm overflow-x-auto scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0">
              ...
              <Link href="/retention" className="text-primary font-medium whitespace-nowrap">Retention</Link>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-1">Cohort Retention</h2>
            <p className="text-sm sm:text-base text-gray-600">Shows the percentage of users from each signup cohort who returned on subsequent days.</p>
          </div>
        ...
        <RetentionHeatmap from={from} to={to} source={source} />
      </main>
    </div>
  );
}
