...
export default function RetentionHeatmap() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      setError(null);
      try {
        const res = await fetch('/api/analytics/retention?from=2026-08-01&to=2026-09-09');
        if (!res.ok) {
  ...
  if (loading) {
    return (
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        <div className="h-96 flex items-center justify-center text-gray-500">Loading retention data...</div>
      </div>
    );
  }
  ...
  if (data.length === 0) {
    return (
      <div className="bg-white p-6 rounded-lg shadow">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Cohort Retention</h3>
        <div className="h-96 flex items-center justify-center text-gray-500">No retention data available for the selected period.</div>
      </div>
    );
  }
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr>
              ...
              <th className="text-left py-2 px-3 font-medium text-gray-500">Users</th>
              {days.map((day) => (
                <th key={day} className="text-center py-2 px-2 font-medium text-gray-500 whitespace-nowrap">
                  Day {day.slice(1)}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {data.map((row) => (
              <tr key={row.period} className="border-t">
                ...
                <td className="py-2 px-3 text-gray-600">{row.newUsers}</td>
                {days.map((day) => {
                  const value = row.retention[day] ?? 0;
                  return (
                    <td key={day} className="py-2 px-2 text-center">
                      <span className={`inline-block w-10 py-1 rounded text-xs font-medium ${getColor(value)} ${getTextColor(value)}`}>
                        {value > 0 ? `${value}%` : '—'}
                      </span>
                    </td>
                  );
